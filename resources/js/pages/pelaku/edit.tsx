...
const formSchema = z.object({
    name: z.string().min(1, 'Pelaku name is required'),
...
export default function EditPelaku({pelaku} : Props) {
    const defaultValues = {
        name: pelaku.name || '',
        tempat_lahir: pelaku.tempat_lahir || '',
        tanggal_lahir: pelaku.tanggal_lahir || '',
        agama: pelaku.agama || '',
        pekerjaan: pelaku.pekerjaan || '',
        jenis_kelamin: (pelaku.jenis_kelamin === 'L' || pelaku.jenis_kelamin === 'P')
            ? pelaku.jenis_kelamin
            : 'L' as const,
        alamat: pelaku.alamat || '',
        status: (pelaku.status === 'lajang' || pelaku.status === 'menikah')
            ? pelaku.status
            : 'lajang' as const,
    };
    ...
    const onSubmit = (data: z.infer<typeof formSchema>) => {
        router.put(`/pelaku/${pelaku.id}`, data);
    };

    return (
        <AppLayout
            breadcrumbs={[
                { title: 'Pelaku', href: '/pelaku' },
                { title: 'Create', href: '#' },
            ]}
        >
            <Head title="Create Pelaku" />
            <div className="m-4 flex max-h-fit max-w-full flex-1 flex-col gap-4 rounded-xl border p-4">
                <h1 className="text-xl font-semibold">Create Pelaku</h1>
