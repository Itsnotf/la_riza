...
export function DataTable<TData, TValue>({ columns, data, page, canCreate , id}: DataTableProps<TData, TValue>) {
    ...
    return (
        <div className="rounded-md border bg-white dark:bg-transparent">
            <div className="flex items-center justify-between p-4">
                ...
                />
                {canCreate && (
                    <Link href={id ? `/${page}/${id}/create` : `/${page}/create`}>
                    <Button className="bg-primary">
                      Create {page.charAt(0).toUpperCase() + page.slice(1)}
                    </Button>
                  </Link>
                )}
            </div>
